...
def add_category_page(
    ...
    return templates.TemplateResponse(request=request, name="admin/add_categories.html")
...
def save_category(
    ...
    if not name:
        return templates.TemplateResponse(
            ...
            context={"error": "Category name is required"},
        )
    ...
    if existing_category:
        return templates.TemplateResponse(
            request=request,
            name="admin/add_categories.html",
            context={"error": "Category already exists", "name": name},
        )
...
def update_category(id: int, name: str = Form(...), db: Session = Depends(get_db)):
    category = db.query(Category).filter(Category.id == id).first()
